...
/** Income — farm income that is not a fish sale. */
function IncomeIndex({
    ...
    const apply = (e) => {
        ...
        setBusy(true);
        router.get(
            routes["finance.income.index"] || window.location.pathname,
            values,
    ...
    const exportHref = routes["finance.income.export"]
        ? exportParams
            ? `${routes["finance.income.export"]}?${exportParams}`
            : routes["finance.income.export"]
        : "";
            ...
            render: (r) =>
                ...
                r.urls?.destroy && (
                    <Button
                        ...
                        onClick={async () => {
                            if (
                                !(await confirm({ title: "Confirm", description: "Delete this income entry? Any linked pond ledger entry will be reversed." }))
                            )
                                return;
    ...
    return (
        <>
            ...
            <PageHeader
                title="Income"
                subtitle="Farm income that is not a fish sale. Each entry is a real recorded figure."
                ...
                actions={
                    <>
                        ...
                        {can("income.create") &&
                            routes["finance.income.create"] && (
                                <Button
                                    href={routes["finance.income.create"]}
                                    variant="secondary"
                                    icon="plus"
                                >
                                    Record Income
            ...
            {Object.keys(byCategory || {}).length > 0 && (
                <div className="mt-5">
                    <Card padded={false} title="Income by category">
...
export default withLayout(IncomeIndex, "Income");
